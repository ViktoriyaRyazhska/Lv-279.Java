drop procedure if exists reportCheckList;
DELIMITER $$
CREATE PROCEDURE reportCheckList(IN academy_id_in int)
BEGIN
	declare teachers tinyint;
	declare experts tinyint;
	declare interviewers tinyint;
	declare approved_by tinyint;
	declare english_level tinyint;
	declare min_english_level tinyint;
	declare entry_score tinyint;
	declare expert_feedback tinyint;
	declare final_base tinyint;
	declare final_lang tinyint;
	declare incoming_test tinyint;
	declare interm_test_base tinyint;
	declare interm_test_lang tinyint;
	declare interviewer_score tinyint;
	declare teacher_feedback tinyint;
	declare experts_load tinyint;
	declare interviewer_load tinyint;
	declare test_1 tinyint;
	declare test_2 tinyint;
	declare test_3 tinyint;
	declare test_4 tinyint;
	declare test_5 tinyint;
	declare test_6 tinyint;
	declare test_7 tinyint;
	declare test_8 tinyint;
	declare test_9 tinyint;
	declare test_10 tinyint;
	...
	call get_teacher_contributed_hours(academy_id_in,2, @experts_contributed_hours);
	call get_teacher_contributed_hours(academy_id_in,3, @interviewers_contributed_hours);
    ...
    IF @teachers <> 0 then set teachers = 1; else set teachers = 0; END IF;
    ...
    IF @experts = 0 then set experts_load = 0;
		elseif @experts - @experts_contributed_hours <> 0 then set experts_load = 0;
        else set experts_load = 1; END IF;
    
    IF @interviewers = 0 then set interviewer_load = 0;
		elseif @interviewers - @interviewers_contributed_hours <> 0 then set interviewer_load = 0;
        else set interviewer_load = 1; END IF;
    ...
    call getTeachers(academy_id_in, 1, @teachers_full_names);
