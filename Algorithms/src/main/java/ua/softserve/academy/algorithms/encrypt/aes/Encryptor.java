...
public class Encryptor {
    ...
    public static String encrypt(String value, String key, byte[] iv) {
        ...
        try {
            return new String(Base64.encodeBase64(json.getBytes("UTF-8")));
    ...
    private static byte[] enCrypt(byte[] value, String key, byte[] iv) {
        ...
        try {
            ...
            cipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(key.getBytes("UTF-8"), "AES"), ivPS);
            res = cipher.doFinal(value);
        } catch (NoSuchAlgorithmException | IllegalBlockSizeException | BadPaddingException | NoSuchPaddingException | InvalidAlgorithmParameterException | InvalidKeyException | UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        return res;
    }
    ...
    private static byte[] deCrypt(String value, String key, byte[] iv) {
        ...
        try {
            final Cipher cipher = Cipher.getInstance("AES/CBC/NoPadding");
            IvParameterSpec ivPS = new IvParameterSpec(iv);
            cipher.init(Cipher.DECRYPT_MODE, new SecretKeySpec(key.getBytes("UTF-8"), "AES"), ivPS);
