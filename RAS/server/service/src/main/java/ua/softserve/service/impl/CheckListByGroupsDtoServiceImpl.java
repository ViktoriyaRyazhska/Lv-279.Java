...
public class CheckListByGroupsDtoServiceImpl implements CheckListByGroupsDtoService {
    ...
    public List<CheckListByGroupsDto> fakeGetAllCheckListByGroupsDto(int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            checkListByGroupsDto.setTeachers(getRandomTeachersInGroup(
                    teachersCount, expertsCount, interviewersCount
            ));
            checkListByGroupsDto.setStudents(getRandomStudentsInGroup(
                    teachersCount, expertsCount, interviewersCount
            ));
    ...
    private CheckListByGroupsDto getRandomGroup() {
        ...
        Date startDate = getRandomDateBetween(
                new Date(System.currentTimeMillis() - ONE_MONTH_IN_MILES * 6),
                new Date(System.currentTimeMillis() + ONE_MONTH_IN_MILES * 6)
        );
        checkListByGroupsDto.setStartDate(startDate);
        checkListByGroupsDto.setEndDate(
                new Date(startDate.getTime() + ONE_MONTH_IN_MILES * 3)
        );
        checkListByGroupsDto.setGroupName(
                getRandomGroupNameByCity(checkListByGroupsDto.getCityName())
        );
        checkListByGroupsDto.setStatus(getRandomGroupStatus());
        return checkListByGroupsDto;
    }

    private CheckListByGroupsDto.TeacherInGroup getRandomTeacherInGroup(
            CheckListByGroupsDto.TeacherTypes type, int teachersCount
    ) {
        CheckListByGroupsDto.TeacherInGroup teacherInGroup =
                new CheckListByGroupsDto().new TeacherInGroup();
    ...
    private List<CheckListByGroupsDto.TeacherInGroup> getRandomTeachersInGroup(
            int teachersCount, int expertsCount, int interviewersCount
    ) {
        List<CheckListByGroupsDto.TeacherInGroup> teachers = new ArrayList<>();
        for (int c = 0; c < teachersCount; c++) {
            teachers.add(getRandomTeacherInGroup(
                    CheckListByGroupsDto.TeacherTypes.TEACHER, teachersCount));
        }
        for (int c = 0; c < expertsCount; c++) {
            teachers.add(getRandomTeacherInGroup(
                    CheckListByGroupsDto.TeacherTypes.EXPERT, expertsCount));
        }
        for (int c = 0; c < interviewersCount; c++) {
            teachers.add(getRandomTeacherInGroup(
                    CheckListByGroupsDto.TeacherTypes.INTERVIEWER, interviewersCount));
        }
        return teachers;
    }

    private List<CheckListByGroupsDto.StudentInGroup> getRandomStudentsInGroup(
            int teachersCount, int expertsCount, int interviewersCount
    ) {
        ...
        for (int j = 0; j < studentsCount; j++) {
            ...
            CheckListByGroupsDto.StudentInGroup studentInGroup =
                    new CheckListByGroupsDto().new StudentInGroup();
            ...
            for (int c = 0; c < teachersCount + expertsCount; c++) {
                CheckListByGroupsDto.TeacherOfStudent teacherOfStudent =
                        new CheckListByGroupsDto().new TeacherOfStudent();
                ...
                for (String s : randomFeedbacks) {
                    if (s.length() > 1) {
                        summary
                                .append(s.toUpperCase().charAt(0))
                                .append(s.toLowerCase().substring(1))
                                .append(" ");
                    } else {
            ...
            CheckListByGroupsDto.TestsOfStudent testsOfStudent =
                    new CheckListByGroupsDto().new TestsOfStudent();
