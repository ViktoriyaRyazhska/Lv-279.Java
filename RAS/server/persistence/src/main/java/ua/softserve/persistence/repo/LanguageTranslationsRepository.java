...
public interface LanguageTranslationsRepository extends JpaRepository<LanguageTranslations, Integer> {
    ...
    @Query(value = "select * from city join language_translations on city.city_id = language_translations.item_id " +
            "where ita = 1 and tag = 'city' and local = 'en' order by trasnlation", nativeQuery = true)
