...
public interface GroupInfoRepository extends JpaRepository<GroupInfo, Integer> {
    ...
    @Query("select new ua.softserve.persistence.dto.GroupInformationDTO(ac.academyId, gi.groupName, gi.studentsPlannedToGraduate, gi.studentsPlannedToEnrollment, ac.startDate, ac.endDate, ac.free, ac.name, pi.profileName, tc.name, acS.name, (select count(stn.academy) from Student as stn where stn.academy = ac.academyId)) " +
            ...
            "left join ac.academyStages as acS left join giT.academy as giTAc " +
            "where giTAc.academyId = ac.academyId order by gi.groupInfoId desc")
